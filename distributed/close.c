...
void closeAlgorithm(Matrix matrix, double minSup, int bounds[2][2], int id) {
    ...
    printf("Thread %d : Starting Gen1\n", id);
    begin = clock();
    ListItem *items = findGen1SupportAndClosure(&listContainer, matrix, minSup, bounds);
    end = clock();
    if (listContainer == NULL) {
        printf("Thread %d : Gen1 time : %lfs.\n", id, (end - begin)/CLOCKS_PER_SEC);
        return;
    }
    ...
    int i = 2;
    do {
        printf("Thread %d : Beginning Gen%d\n",  id, i);
        begin = clock();
        listContainer = findNextGenerators(listContainer);
        if (listContainer == NULL) {
            printf("Thread %d : Gen%d time : %lfs.\n", id,  i, (end - begin)/CLOCKS_PER_SEC);
            sum += (end - begin);
            break;
        }
        ...
        while (iterator != NULL) {
            findSupportAndClosure(&iterator->container, matrix, minSup, items, bounds);
    ...
    printRulesInFile(findRules(head), id);

    printf("Thread %d : Total time : %lfs.\n", id,  sum/CLOCKS_PER_SEC);
}
...
ListRule *findRules(ListListContainer *listListContainer) {
    ...
    while (head != NULL) {
        ...
        while (listContainer != NULL) {
            ...
            while (generator != NULL && closure != NULL) {
...
void findSupportAndClosure(Container *container, Matrix matrix, double minSup, ListItem *sortedItems, int bounds[2][2]) {
    ...
    BitmapPosition start1 = findBitmapPosition(bounds[0][0]);
    BitmapPosition end1 = findBitmapPosition(bounds[0][1]);
    BitmapPosition start2 = findBitmapPosition(bounds[1][0]);
    BitmapPosition end2 = findBitmapPosition(bounds[1][1]);
    ...
    if (container->support >= minSup) {
        ...
        for (int dim = 0; dim < 2; dim++) {
            for (j = bounds[dim][0]; j <= bounds[dim][1]; j++) {
...
ListItem *findGen1SupportAndClosure(ListContainer **pListContainer, Matrix matrix, double minSup, int bounds[2][2]) {
    ...
    int i, j, count;
    for (int dim = 0; dim < 2; dim++) {
        for (int var = bounds[dim][0]; var <= bounds[dim][1]; var++) {
            i = 0;
            count = 0;
            Item currentItem;
            ...
            if (currentItem.support >= minSup) {
                ...
                for (int dim2 = 0; dim2 < 2; dim2++) {
                    for (j = bounds[dim2][0]; j <= bounds[dim2][1]; j++) {
                        position = findBitmapPosition(j);
                        if ((row[position.cell] & position.bit) != 0) {
                            //The item is part of the closure.
                            currentItemset = (Itemset *) malloc(sizeof(Itemset));
...
void printRulesInFile(ListRule *rules, int id) {
    char filename[100];
    sprintf(filename, "rules%d.txt", id);
